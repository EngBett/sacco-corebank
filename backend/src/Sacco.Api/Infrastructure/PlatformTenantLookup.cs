...
{
    public async Task<(Guid Id, string Slug, string Name, string PrimaryColor, string? LogoUrl, string? FaviconUrl)?> FindBySlugAsync(string slug, CancellationToken ct)
    {
        var t = await directory.FindBySlugAsync(slug, ct);
        return t is null || !t.IsActive ? null : (t.Id, t.Slug, t.Name, t.Branding.PrimaryColor, t.Branding.LogoUrl, t.Branding.FaviconUrl);
    }
}
